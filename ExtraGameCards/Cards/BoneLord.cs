...
namespace EGC.Cards
{
    public class BoneLord : CustomCard
    {
            ...
            HealthHandler health, Gravity gravity, Block block, CharacterStatModifiers characterStats)
        {
            ...
            ExtraGameCards.Instance.StartCoroutine(RemoveRandomCardOfPlayer(player));
        }
        ...
        private IEnumerator RemoveRandomCardOfPlayer(Player player, int numberOfCardToRemove = 1)
        {
            ...
            for (int i = 0; i < numberOfCardToRemove; i++)
            {
                ...
                while (!(tries > 50))
                {
                    ...
                    if (!instance.CardIsNotBlacklisted(card,
                            new[]
                            {
                    ...
                    if (!instance.PlayerIsAllowedCard(player, card))
                    {
                        continue;
                    }

                    UnityEngine.Debug.Log("Trying to remove : " + card.cardName);
                    yield return instance.RemoveCardFromPlayer(player, playerCards[randomCardIdx],
                        SelectionType.Oldest);
